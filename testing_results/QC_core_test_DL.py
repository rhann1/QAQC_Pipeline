...
def main(IsSubHourly, scriptId):
    ...
    if testMode:
        ...
        measurementFrame, configFrame = dh.getData(True, intervalHoursForSubHourly, maxStreamLimit, maxCycles, MaxRecords) # getData() returns tuple of dataframes.  Passed argument is 'IsSubHourly'.
        ...
        print("execution time = " + str(end_processing - start_processing))
        ...
        beginGetData = time.time()
        measurementFrame, configFrame = dh.getData(False, intervalHoursForHourly, maxStreamLimit, maxCycles, MaxRecords) # getData() returns tuple of dataframes.  Passed argument is 'IsSubHourly'.
        endGetData = time.time()
        #computedQFlagFrame, subHourlyAggregations = QC_Core(False, measurementFrame, configFrame) # QC flags
        print("number of total records retrieved = " + str(len(measurementFrame)))
        ...
        batchId = dh.GetBatchId(QaScriptId, datetime.now(), len(measurementFrame)) # get QaProcessingLogId
        start_processing = time.time()
        print(measurementFrame)
        processedFrames = QC_Core(False, False, measurementFrame, configFrame) # QC flags
...
def QC_Core(testMode, IsSubHourly, measurementFrame, configFrame):
    ...
    for group in gp:
        ...
        if len(configData.loc[configData['StreamSegmentId'] == group[0]].values) != 0:
            print('Processing StreamSegmentId = ' + str(group[0]) + ' records processed = ' + str(len(pd.DataFrame(group[1]))))
            ...
            frame['CurrentIC'] = frame['IsCalculated']  # this was working to avoid assertion error 
            #frame['IsCalculated'] = 1   #put this back if nothing else works            
            #frame['IsCalculated'] = [1 if math.isnan(i) else i for i in frame['IsCalculated']]
            frame['IsCalculated'].fillna(value = np.nan, inplace=True)
            frame['IsCalculated'].fillna(value = 1, inplace=True)
    ...
    df_result[['QF01', 'QF02', 'QF03', 'QF04',
       'QF05', 'QF06', 'QF07', 'QA_overall', 'IsCalculated']] = df_result[['QF01', 'QF02', 'QF03', 'QF04',
       'QF05', 'QF06', 'QF07', 'QA_overall', 'IsCalculated']].astype(int)
    
    # end function drivers
    if IsSubHourly:
        return df_result,0 #, adf_result commenting out aggregation result until assertion error is resolved
    else:
        return df_result
