...
def QC_Core(IsSubHourly, measurementFrame, configFrame):
    ...
    for group in gp:
        ...
        if len(configData.loc[configData['StreamSegmentId'] == group[0]].values) != 0:
            ...
            dfspike = list(np.array([int(x) for x in pd.Series(df2).fillna(-2).tolist()]) & \
                           np.array([int(x) for x in pd.Series(df3).fillna(-2).tolist()]) & \
                           np.array([int(x) for x in pd.Series(df4).fillna(-2).tolist()]))
...
if __name__ == "__main__":
    ...
    if testMode:
        ...
        configData = tds.getQAConfigDataFromFile('sample_QA_metadata/QC_Metadata_SM_OneMonth.csv')
        configData.replace(r'^\s*$', np.nan, regex=True, inplace=True) # replace any white space with NaN. Unwanted whitespace instead of NULL is sometimes generated by Excel
        start = time.time()
